...
export const maxExpeditionSlots = (state: GameState): number => 1 + talentLevel(state, 'expeditionCorps')
...
export function collectExpedition(
  ...
  if (rolls[0] < EXPEDITION_FAIL_CHANCE) {
    const insured = talentLevel(current, 'insurance') > 0
    const lost = insured ? 0 : Math.floor(expedition.drones / 2)
    current = {
      ...current,
  ...
  const ore = perDrone * expedition.drones * def.durationSec * def.rewardScale
  const rare = rolls[0] < EXPEDITION_FAIL_CHANCE + EXPEDITION_RARE_CHANCE
  const gains: Partial<Resources> = rare
    ? { ore, alloy: ore * 0.2, chip: ore * 0.02, core: ore * 0.001 }
    : { ore, alloy: ore * 0.1 }
  const dm = rare && rolls[1] < EXPEDITION_DM_CHANCE ? 1 : 0
