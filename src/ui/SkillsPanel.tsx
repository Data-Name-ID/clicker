            ...
            def.requires.map((req) => {
              ...
              return (
                <line
                  ...
                  x2={to.x}
                  y2={to.y}
                  className={`sky__line ${lit ? 'sky__line--lit' : half ? 'sky__line--half' : ''}`}
                />
              )
            }),
          )}
        ...
        {nodes.map((def) => {
          ...
          return (
            <button
              type="button"
              key={def.id}
              className={`star star--${st} ${selected === def.id ? 'star--selected' : ''}`}
...
export function SkillsPanel() {
  ...
  return (
    <div className="panel-body">
      ...
      <p className="muted">
        Кликни по звезде — карточка откроется рядом. Навыки постоянны, их не сжигает даже Галактика.
      </p>
